...
void emit_particle(ParticleSystem* ps, vec3 pos, vec3 vel, vec4 color, float life, float size) {
    int unused = first_unused_particle(ps);
    glm_vec3_copy(pos, ps->particles[unused].position);
    glm_vec3_copy(vel, ps->particles[unused].velocity);
    glm_vec4_copy(color, ps->particles[unused].color);
    ps->particles[unused].life = life;
    ps->particles[unused].max_life = life;
    ps->particles[unused].size = size;
}
