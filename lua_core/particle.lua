...
addEventHandler("onRender", function()
    for id, fire in pairs(activeFires) do
        for i = 1, fire.density do
            local spawnX = fire.x + (math.random() - 0.5) * 0.4 * fire.scale
            local spawnY = fire.y + (math.random() - 0.5) * 0.4 * fire.scale
            local spawnZ = fire.z 

            local vx = (math.random() - 0.5) * 0.2 * fire.scale
            local vy = (math.random() - 0.5) * 0.2 * fire.scale
            local vz = (math.random() * 1.5 + 1.0) * fire.scale 

            local life = math.random() * 0.4 + 0.4
            
            local size = (math.random() * 0.6 + 0.4) * fire.scale
